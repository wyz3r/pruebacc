...
import {createProject, getProjects} from './proyectos'
...
describe('getprojects ', () => {
  it('Prueba de happypath para getproject ', async () => {
    const mockUserInfo = {uid: 'RRIYM7MYVcRQwH5lsuCkd8f1T9H2'}
    const result = await getProjects(mockUserInfo)
    expect(cassSelectDB).toHaveBeenCalledTimes(2)
    expect(cassSelectDB).toHaveBeenCalledWith('SELECT * FROM analyzer.user_projects  WHERE user_id = ?;', expect.any(Array))
    expect(cassSelectDB).toHaveBeenCalledWith('SELECT * FROM analyzer.projects;')
    // expect(cassSelectDB).toHaveBeenCalledWith('SELECT * FROM analyzer.projects')
    expect(result).toEqual(expect.any(Object))
  })

  it('Prueba de happypath para comprobar el error de getproject ', async () => {
    try {
      await getProjects()
      expect(cassSelectDB).not.toHaveBeenCalled()
    } catch (error) {
      expect(error.TypeError).toEqual(undefined)
    }
  })
